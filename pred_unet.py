...
def locate_and_correct(img_src, img_mask):
    ...
    # cv2.waitKey(0)
    # print('img_mask',img_mask[:, :, 0].shape)
    try:
    ...
    if not len(contours):  # contours1长度为0说明未检测到车牌
        ...
        for ii, cont in enumerate(contours):
            ...
            if np.mean(img_cut_mask) >= 50 and w > 15 and h > 15:
...
# model.eval()
# img_src_path = 'data/image/京A10000.jpg'
# img_src, img_mask = unet_predict(model, img_src_path)
# img_src_copy, Lic_img = locate_and_correct(img_src, img_mask)
# Lic_img = np.asarray(Lic_img, dtype=np.uint8).reshape((80, 240, 3))
#
#
